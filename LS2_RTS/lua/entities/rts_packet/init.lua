...
function ENT:TransferResources( restype, resamount )
    ...
    elseif (restype == "redterracrystal") then --Red Terra Crystal
    	...
    	local trail = util.SpriteTrail(self, 0, Color(255,0,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
    	self.damagemultiplier = 5
    elseif (restype == "greenterracrystal") then --Green Terra Crystal
		self:SetColor(Color(0,255,0,255))
    	local trail = util.SpriteTrail(self, 0, Color(0,255,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
    	self.damagemultiplier = 7
    elseif (restype == "terrajuice") then --Terrajuice
		self:SetColor(Color(0,0,0,255))
    	local trail = util.SpriteTrail(self, 0, Color(0,0,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
...
function ENT:PhysicsCollide(data, physobj )
	...
	if class and class == "rts_massdriver" then
		...
		rts_Explosion( self.resourceamount * self.damagemultiplier / 4,(5 + self.damagemultiplier/10) , self.resourceamount * self.damagemultiplier / 2,self:GetPos()+  self:GetUp() * 25,self.Activator)
