...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( vOffset )
		...
		if (shockwave) then
			...
			shockwave:SetStartSize( scale )
			shockwave:SetEndSize( 150 * scale )
		...
		for i=0, 64 do
			...
			if (shockwave2) then
				...
				shockwave2:SetStartSize( 3 * scale )
				shockwave2:SetEndSize( 3.5 * scale )
		...
		for i=0, NumParticles do
			...
			if (particle) then
				...
				particle:SetStartSize( 0.3 * scale )
				particle:SetEndSize( 0.15 * scale )
			...
			if (particle2) then
				...
				particle2:SetStartSize( 1.5 * scale )
			...
			if (particle3) then
				...
				particle3:SetStartSize( 1.5 *  scale )
				particle3:SetEndSize( 1.2 * scale  )
			...
			if (particle4) then
				...
				particle4:SetStartSize( 2 * scale )
				particle4:SetEndSize( 1.2 * scale)
...
function ParticleThink( part )
	...
	if part:GetLifeTime() > 0.18 and part:GetLifeTime() < 1 then
		...
		if (particle) then
			...
			particle:SetStartSize( ((scale + math.Rand(5,10)) - (part:GetLifeTime() * scale)))
			particle:SetEndSize( (((scale * 1.2) + math.Rand(5,10)) - (part:GetLifeTime() * scale)))
