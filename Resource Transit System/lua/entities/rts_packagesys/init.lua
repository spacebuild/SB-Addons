...
function ENT:TriggerInput(iname, value)
	...
	if(iname == "Compress") then
		if((value == 1) and (self.reloadtime < CurTime())) then
			if (RD.GetResourceAmount(self, "energy") >  self.energyuse) then
				self.charging = 1
				self.counter = 0
...
function ENT:Think()
	...
	local tempvalue = RD.GetResourceAmount(self, tempvarname)
...
---------------------WIRE MOD OUTPUTS------------------------------------------------- 
	if not (WireAddon == nil) then 
			Wire_TriggerOutput(self, "Ready", tonumber((self.reloadtime < CurTime())))
