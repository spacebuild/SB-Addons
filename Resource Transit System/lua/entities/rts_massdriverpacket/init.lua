...
function ENT:HealthCheck(ExplodeMod)
	if self.health <= CDSFudgeFix then
		...
		local RTS = CAF.GetAddon("Resource Transit System")
		RTS.Explosion( self.ResAmount * self.damagemultiplier / 20 * ExplodeMod,2 + self.damagemultiplier + (ExplodeMod * 10) , self.ResAmount * self.damagemultiplier / 10 * ExplodeMod,self:GetPos()+  self:GetUp() * 25,self.Activator)
		self:Remove()
	end
end
...
function ENT:PhysicsCollide(data, physobj )
	...
	if class and class == "rts_massdriver" then
		...
		if ( self.Velocity > _Threshold) then
			--rts_Explosion	(damage												, piercing						, area												, position											, killcredit	)
			local RTS = CAF.GetAddon("Resource Transit System")
			RTS.Explosion	( self.ResAmount * self.damagemultiplier / 4	,(5 + self.damagemultiplier/10) , self.ResAmount * self.damagemultiplier / 2	, self:GetPos()+  self:GetUp() * 25	, self.Activator)
			self:Remove()
		end
	end
end
