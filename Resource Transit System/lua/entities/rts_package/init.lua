...
function ENT:HealthCheck(ExplodeMod)
	if self.health <= CDSFudgeFix then
		...
		RTS.Explosion( self.resourceamount * self.damagemultiplier / 20 * ExplodeMod,2 + self.damagemultiplier + (ExplodeMod * 10) , self.resourceamount * self.damagemultiplier / 10 * ExplodeMod,self:GetPos()+  self:GetUp() * 25,self.Activator)
		self:Remove()
	end
end
