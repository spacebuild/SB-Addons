...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self, { "On", "Steam Consumption" })
end
...
function ENT:CanRun()
 ...
 function ENT:Think()
	...
	if (self:IsOn() and self:CanRun()) then
		RD_ConsumeResource(self, "steam", self.steamcon)
		self.outputon = 1
	else
		self:Switch( false )
		self.outputon = 0
	...
	if not (WireAddon == nil) then
		Wire_TriggerOutput(self, "Steam Consumption", self.steamcon)
