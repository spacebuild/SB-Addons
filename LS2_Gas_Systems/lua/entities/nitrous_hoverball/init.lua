...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self, { "Zpos", "Xpos", "Ypos", "Speed", "Strength", "Resistance", "NitrousOxideConsumption" })
end
...
function ENT:Think()
    ...
    if (self:CanRun() and self:GetHoverMode() ) then
        RD_ConsumeResource(self, "nitrous", self.nitrouscon)
    else
        self:DisableHover()
	self.nitrouscon = 0
    end
    ...
    Wire_TriggerOutput(self, "NitrousOxideConsumption", self.nitrouscon)
	return true
end
