...
function ENT:Initialize()
	self:SetModel("models/props/de_nuke/equipment2.mdl")
	...
	if not (WireAddon == nil) then self.Outputs = Wire_CreateOutputs(self, { "On", "Overdrive", "Energy Consumption", "Nitrogen Consumption", "Nitrous Oxide Production"}) end
...
function ENT:OxNitro()
    ...
    if not (WireAddon == nil) then
        ...
        Wire_TriggerOutput(self, "Nitrogen Consumption", self.nitrocon)
        Wire_TriggerOutput(self, "Nitrous Oxide Production", self.nitrousprod)
    end
	...
	if ( self:CanRun() ) then
        ...
        RD_SupplyResource(self, "nitrous",self.nitrousprod)
