include('shared.lua')
language.Add("asteroid_scanner", "Asteroid Scanner")
	...
	if ( LocalPlayer():GetEyeTrace().Entity == self and EyePos():Distance( self:GetPos() ) < rd_overlay_dist and mode ~= 0) then
		...
		if not mode or mode ~= 2 then
			...
			if self:GetOOO() == 1 then
				OverlayText = OverlayText .. "Asteroid Information:\n"
				OverlayText = OverlayText .. "Availible Resource Ammount: "..tostring(self:GetNetworkedInt( "Mineable Ammount" )).."\n"
			end
			...
			cam.Start3D2D(pos,angle,0.03)
					...
					surface.SetTextColor(155,155,255,255)
					surface.SetTextPos(textStartPos+15,TempY)
					surface.DrawText("Resource Ammount: " .. string.format("%g",self:GetNetworkedInt( "Mineable Ammount" )))
			--Stop rendering
			cam.End3D2D()
		end
	else
